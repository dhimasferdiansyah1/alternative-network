...
import { transactionStorage } from "@/app/api/create-transaction/route";
...
export default function PaymentStatus() {
  ...
  const executeRconCommand = async () => {
    ...
    if (!transactionDetails || !username) return false;

    const amount = transactionDetails.gross_amount;

    // Ambil nama item dari transactionStorage menggunakan orderId
    const storedTransaction = transactionStorage[transactionDetails.order_id];
    const itemNames = storedTransaction
      ? storedTransaction.items.map((item) => item.name).join(", ")
      : "Unknown Item"; // Pastikan ini tidak terjadi
  ...
  return (
    <div className="min-h-screen bg-neutral-900 flex items-center justify-center">
      <div className="bg-neutral-800 p-8 rounded-lg shadow-md text-white max-w-md w-full">
        ...
        {status === "Payment successful" && username && (
          <div className="mt-6">
            <button
